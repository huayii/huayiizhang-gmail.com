...
class TestLesson(TransactionCase):
    ...
    def test_lesson(self):
        ...
        lesson = self.env['pscloud.training.lesson'].create({
            'name': '英语协作',
            'teacher_id': self.env.ref('tranining.demo_training_subject').id,
            'start_date': '2018-01-01',
            'end_date': '2018-07-01',
            'seat_qty': 30,
            'subject_id': self.env.ref('tranining.demo_training_subject').id,
            'student_ids': [(4, self.env.ref('tranining.demo_training_student_1').id)],
        })
